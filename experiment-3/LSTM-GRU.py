...
model = Sequential()
model.add( LSTM(INPUT_LEN, input_shape=(trainX.shape[1:]), return_sequences=True, activity_regularizer=REGULARIZATION) )
model.add( Dropout(DROPOUT) )
model.add( BatchNormalization() )

model.add( GRU(INPUT_LEN, return_sequences=False, activity_regularizer=REGULARIZATION) )
model.add( Dropout(DROPOUT) )
model.add( BatchNormalization() )
