...
from tensorflow.keras.layers import SimpleRNN, Dense, Bidirectional, Input, Dropout, BatchNormalization, TimeDistributed
...
model = Sequential()
model.add( SimpleRNN(INPUT_LEN, input_shape=(trainX.shape[1:]), return_sequences=True, activity_regularizer=REGULARIZATION) )
model.add( Dropout(DROPOUT) )
model.add( BatchNormalization() )

model.add( SimpleRNN(INPUT_LEN, return_sequences=False, activity_regularizer=REGULARIZATION) )
model.add( Dropout(DROPOUT) )
model.add( BatchNormalization() )
...
earlyStopping = EarlyStopping(monitor='val_loss', min_delta=0, patience=30, verbose=1, mode='min', baseline=None, restore_best_weights=True)
reduceOnPlateau = ReduceLROnPlateau(monitor='val_loss', factor=0.6, patience=6, min_lr=0.001)
