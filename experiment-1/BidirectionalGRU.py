...
for layer in LAYERS:
    for s in SIZE:
        for l in LEARNER:
            ...
            # Build Model
            model = Sequential()
            model.add( Bidirectional(GRU(s, input_shape=(trainX.shape[1:]), return_sequences=True)) )
            ...
            name = f"BidirectionalGRU-SEQ{SEQUENCE_LENGTH}-Layers{layer}-Size{s}-RMSprop-Regression-{int(time.time())}"
