...
for layer in LAYERS:
    for s in SIZE:
        for l in LEARNER:
            ...
            # Build Model
            model = Sequential()
            model.add( Bidirectional(LSTM(s, input_shape=(trainX.shape[1:]), return_sequences=True)) )
            model.add( Dropout(0.2) )
            model.add( BatchNormalization() )
            
            for eachLayer in range(layer-2):
                model.add( Bidirectional(LSTM(s, return_sequences=True)) )
                model.add( Dropout(0.2) )
                model.add( BatchNormalization() )
            
            model.add( Bidirectional(LSTM(s, return_sequences=False)) )
            model.add( Dropout(0.2) )
            model.add( BatchNormalization() )
            ...
            name = f"BidirectionalLSTM-SEQ{SEQUENCE_LENGTH}-Layers{layer}-Size{s}-RMSprop-Regression-{int(time.time())}"
